      ...
      >
        ...
        <template #item.count="{ item }">
          <v-btn
            variant="text"
            icon="mdi-minus"
            :disabled="item.columns.count == 1"
            @click.prevent="minusHandler(item.selectable.skuId, item.columns.count)"
          ></v-btn>
          <v-btn variant="text" :ripple="false">{{ item.columns.count }}</v-btn>
          <v-btn
            variant="text"
            icon="mdi-plus"
            @click.prevent="plusHandler(item.selectable.skuId, item.columns.count)"
          ></v-btn>
        </template>
        ...
        <template #item.operator="{ item }">
          <v-btn color="green" variant="text" @click.prevent="deleteHandler(item.selectable.skuId)"
            >删除</v-btn
          >
        </template>
      </v-data-table>
