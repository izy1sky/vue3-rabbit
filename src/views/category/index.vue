  ...
  <v-card variant="flat">
    <v-card-item>
      <v-breadcrumbs divider=">" 
        ...
        :items="breadCrumbsItems"> </v-breadcrumbs>
    </v-card-item>
    <v-card-item>
      ...
      <v-card-item>
        <v-row justify="center" align="stretch">
          <template v-for="children in childrenItems" :key="children.id">
            <v-col>
              <v-card height="100%" variant="flat">
    ...
    <v-card-item v-if="childrenItems.length">
      <template v-for="children in childrenItems" :key="children.id">
        <v-card variant="flat" class="mt-5">
          <v-card-item class="text-h4 font-weight-bold text-center">{{ children.name }}</v-card-item>
          <v-card-item>
            <v-row justify="center" align="stretch">
              <template v-for="good in children.goods" :key="good.id">
                <v-col cols="2" class="ma-2">
                  <GoodsItemCard :picture="good.picture"
                  :name="good.name" :desc="good.desc" :price="good.price"></GoodsItemCard>
                </v-col>
              </template>
            </v-row>
          </v-card-item>
