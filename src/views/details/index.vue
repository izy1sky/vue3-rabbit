        ...
        <v-row justify="center" align="stretch">
                ...
                <v-row justify="center" align="stretch">
                    <v-col cols="10">
                        <div class="father-box" style="position: relative;">
                            <v-img class="big-image" :src="imgUrl" cover @mousemove.stop="mousemoveHandler" @mouseleave.stop="mouseleaveHandler"></v-img>
                ...
                <v-row justify="center" align="stretch">
                    <v-col cols="3">
                        <v-card variant="flat">
                            ...
                            <v-card-item class="text-center text-red text-h6 font-weight-bold">{{ goodInfo.salesCount+'+' }}</v-card-item>
                    ...
                    <v-col cols="3">
                        <v-card variant="flat">
                            ...
                            <v-card-item class="text-center text-red text-h6 font-weight-bold">{{ goodInfo.commentCount+'+' }}</v-card-item>
                            <v-card-actions class="justify-center">
                    ...
                    <v-col cols="3">
                        <v-card variant="flat">
                            ...
                            <v-card-item class="text-center text-red text-h6 font-weight-bold">{{ goodInfo.collectCount+'+' }}</v-card-item>
                            <v-card-actions class="justify-center">
                    ...
                    <v-col cols="3">
                        <v-card variant="flat">
                            ...
                            <v-card-item class="text-center text-red text-h6 font-weight-bold">{{ goodInfo.brand.name }}</v-card-item>
                            <v-card-actions class="justify-center">
            ...
            <v-col cols="6" class="d-flex flex-column align-start justify-start text-body-1">
                <v-card variant="flat" width="100%">
                    <v-card-item><div class="font-weight-bold">{{ goodInfo.name }}</div></v-card-item>
                    <v-card-item><div class="font-weight-light">{{ goodInfo.desc }}</div></v-card-item>
                    <v-card-item>
                        <div>
                            <span class="font-weight-bold text-red text-body-6">{{ `￥${goodInfo.price}` }}</span>
                            <span class="ms-2 text-caption text-decoration-line-through">{{ `￥${goodInfo.oldPrice}` }}</span>
                        </div>
                    </v-card-item>
                ...
                <v-card variant="tonal" width="100%">
                    <v-card-item>
                        <span class="text-caption">促销</span> <span class="ms-2">12月好物放送，App内购买直降120元</span>
                    </v-card-item>
                    <v-card-item class="align-center">
                        <span class="text-caption">服务</span> <span class="ms-2">·无忧退货&nbsp;&nbsp;·快速退款&nbsp;&nbsp;·免费包邮</span><span class="ms-5 text-blue">了解详情</span>
                    </v-card-item>
                </v-card>
                <SpecCard class="mt-5" :specs="goodInfo.specs" :skus="goodInfo.skus" @change="changeSpecsHandler"></SpecCard>
            </v-col>
        </v-row>
    </v-card>
    ...
    <v-row justify="center" align="start" class="mt-5" v-if="goodInfo">
        ...
        <v-col cols="2" class="pa-2">
            <v-card>
                ...
                <GoodsItemCard class="my-5" v-for="good in goodInfo.hotByDay" :key="good.id"
            :picture="good.picture" :name="good.name" :desc="good.desc" :price="good.price"></GoodsItemCard>
            </v-card>
            ...
            <v-card class="mt-10">
                ...
                <GoodsItemCard class="my-5" v-for="good in goodInfo.similarProducts" :key="good.id"
            :picture="good.picture" :name="good.name" :desc="good.desc" :price="good.price"></GoodsItemCard>
            </v-card>
        </v-col>
    </v-row>
...
const mouseoverHandler = ($event:Event) => {
    ...
    document.querySelectorAll('div.selected-box').forEach((el) => el.classList.toggle('selected-box'))
    ($event.target as HTMLElement).classList.add('selected-box')
}
...
const mousemoveHandler = (event:MouseEvent) => {
    ...
    // 当偏移量处于合理范围内时，赋值
    const width = Math.min(Math.max(relativeX - 0.5 * +transparentBoxWidth, 0), rect.width - transparentBoxWidth)
    const height = Math.min(Math.max(relativeY - 0.5 * +transparentBoxHeight, 0), rect.height - transparentBoxHeight)
