...
export const reqDeleteCart = (ids: string[]) =>
  request.delete<any, DeleteCartResponseData>(CART_API.DELETE_ADD_GET, {
    data: {
      ids
    }
  })

export const reqAddCart = (data: { skuId: string; count: number }) =>
  request.post<any, AddCartResponseData>(CART_API.DELETE_ADD_GET, data)

export const reqUpdateCart = (skuId: string, selected: boolean, count: number) =>
  request.put<any, AddCartResponseData>(CART_API.UPDATE + `/${skuId}`, {
    selected,
    count
  })
